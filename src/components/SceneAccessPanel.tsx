...
type SceneAccessPanelProps = {
  ...
  /** Renders body only (no outer side-panel shell) for tabbed sidebars. */
  embedded?: boolean;
};
...
/// </summary>
export function SceneAccessPanel({ state, dm, embedded = false }: SceneAccessPanelProps) {
      ...
      {state.playerSlots.length === 0 ? (
        ...
        <ul className="scene-access-list">
          {state.playerSlots.map((slot) => {
            const canSee = activeScene ? slot.visibleSceneIds.includes(activeScene.id) : false;
            return (
              <li key={slot.id}>
  ...
  if (embedded) {
    return <div className="scene-access-embedded">{body}</div>;
  }
